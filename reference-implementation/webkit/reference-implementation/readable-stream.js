...
var test22 = async_test('ReadableStreamController methods should continue working properly when scripts lose their reference to the readable stream');
...
var test23 = async_test('ReadableStream closed promise should fulfill even if the stream and reader JS references are lost');
test23.step(function() {
    ...
    }).getReader().closed.then(test23.step_func(function() {
        test23.done('closed promise should be fulfilled');
    }));
...
var test24 = async_test('ReadableStream closed promise should reject even if stream and reader JS references are lost');
test24.step(function() {
    ...
    }).getReader().closed.catch(test24.step_func(function() {
        test24.done('closed promise should be rejected');
    }));

    garbageCollectAndDo(test24.step_func(function() {
        controller.error();
    }));
});
